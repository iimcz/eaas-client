...
export class ComponentSession extends EventTarget {
    ...
    hasQemuControl() {
        return this.connectQemuUrl !== null;
    }

    fetchQemuDevices() {
        fetch(this.connectQemuUrl, {
            method: 'GET'
        }).then(response => response.json())
        .then(data => this.qemuDevices = data);
    }

    async connectKeyboard() {
        const command = this.qemuDevices.reduce(
            (cmd, dev) => {
                if (dev.deviceType === 'keyboard') {
                    cmd += dev.connectCommand;
                    cmd += '\n';
                }
                return cmd;
            }, ""
        ...
        console.log(command);
        const res = await fetch(this.connectQemuUrl, {
            method: 'POST',
            body: command
        });
    }

    async disconnectKeyboard() {
        const command = this.qemuDevices.reduce(
            (cmd, dev) => {
                if (dev.deviceType === 'keyboard') {
                    cmd += dev.disconnectCommand;
                    cmd += '\n';
                }
                return cmd;
            }, ""
    ...
    async _getControlUrl() {
        try {
            ...
            if (result.qemu) {
                this.connectQemuUrl = result.qemu;
                this.fetchQemuDevices();
            }
    ...
    async release() {
        ...
        if (this.isEphemeral) {
            ...
            console.log(`Released ephemeral session ${this.getId()}`);
        } else console.log(`Skipped background session ${this.getId()}`);
    ...
    async _initWebRtcAudio(url) {
        ...
        */
        ...
        this.rtcPeerConnection.onaddstream = async (event) => {
            ...
            this._audioStream = event.stream;
